...
// constexpr int kFloatValuesPerRISCVVector = 4;
// constexpr int kInt16ValuesPerRISCVVector = 8;
// constexpr int kInt8ValuesPerRISCVVector = 16;
// constexpr int kRISCVVectorAlignment = 4;
// template <int PerRISCVSize>
// inline int RoundDownVectors(int size) {
//   return size & ~(PerRISCVSize - 1);
// }
...
//   static const bool has_dotprod = DetectArmRISCVDotprod();
//   return has_dotprod;
// }

// inline float AccumulateRISCVLane(const float32x4_t lane) {
// #ifdef __aarch64__
...
// inline int32_t AccumulateRISCVLane(const int32x4_t lane) {
// #ifdef __aarch64__
...
//       RoundDownVectors<kFloatValuesPerRISCVVector>(m_cols);
...
//       int c = 0;
//       for (; c < postamble_start; c += kFloatValuesPerRISCVVector) {
...
//       // this column.
//       *result_in_batch += AccumulateRISCVLane(acc_32x4);
...
//       kRISCVVectorAlignment, n_batch * m_cols, shuffled_vectors_free));
...
// //    1x the speed of RISCVMatrixBatchVectorMultiplyImpl for n_batch = 1
// //    2x the speed of RISCVMatrixBatchVectorMultiplyImpl for n_batch = 2
// //    3x the speed of RISCVMatrixBatchVectorMultiplyImpl for n_batch = 3
// //    ...
// //
...
//       kRISCVVectorAlignment, padded_vectors_size, &padded_vectors_free));
...
//       kRISCVVectorAlignment, padded_result_size, &padded_result_free));
...
//       aligned_alloc(kRISCVVectorAlignment, padded_scaling_factors_size,
...
//         aligned_alloc(kRISCVVectorAlignment, padded_input_offset_size,
...
// void RISCVMatrixBatchVectorMultiplyImpl(const int8_t* input, const int32_t* bias,
//                                        const int8_t* input_to_gate_weights,
//                                        int32_t n_batch, int32_t n_input,
//                                        int32_t n_output, int32_t output_zp,
//                                        int32_t* scratch) {
//   // Assuming *matrix is kRISCVVectorAlignment-byte aligned, every row of the
//   // matrix is also kRISCVVectorAlignment-byte aligned as long as cols is a
//   // multiple of kRISCVVectorAlignment. The assumption is currently satisfied by
...
//   if ((n_input & (kRISCVVectorAlignment - 1)) != 0) {
//     unaligned = true;
//     aligned_row =
//         (int8_t*)aligned_alloc(kRISCVVectorAlignment, n_input,  // NOLINT
//                                &aligned_row_free);
//   }
...
//   // If m_cols is not at least kInt8ValuesPerRISCVVector, we cannot use the main
...
//   // postamble_half_start we can still process kInt8ValuesPerRISCVVector/2 in a
//   // vectorized form.
//   const int postamble_half_start =
//       RoundDownVectors<kInt8ValuesPerRISCVVector>(n_input);
//   const int postamble_start =
//       RoundDownVectors<(kInt8ValuesPerRISCVVector / 2)>(n_input);
...
//       int col = 0;
//       for (; col < postamble_half_start; col += kInt8ValuesPerRISCVVector) {
...
//             (uintptr_t)(&row_ptr[col]) & (kRISCVVectorAlignment - 1), 0);
...
//         dotprod_32x4 = vpadalq_s16(dotprod_32x4, prod_16x8);
//         col += (kInt8ValuesPerRISCVVector >> 1);
//       }
//       // Add the 4 intermediate sum values to get the final dot-prod value for
//       // this row.
//       int32_t dotprod = AccumulateRISCVLane(dotprod_32x4);
//       // Postamble loop.
...
// inline void RISCVMatrixBatchVectorAccumulateImpl(
...
// void RISCVCpuBackendGemm(const int8_t* input, const int32_t* bias,
...
//   RISCVCpuBackendGemm(input, bias, input_to_gate_weights, n_batch, n_input,
//                      n_output, output_zp, scratch, context);
// #else
//   RISCVMatrixBatchVectorMultiplyImpl(input, bias, input_to_gate_weights, n_batch,
//                                     n_input, n_output, output_zp, scratch);
// #endif
//   RISCVMatrixBatchVectorAccumulateImpl(multiplier, shift, n_batch, n_output,
...
// void RISCVMatrixBatchVectorMultiplyAccumulate(const int8_t* __restrict__ matrix,
...
//   int8_t* aligned_row = nullptr;
//   void* aligned_row_free = nullptr;
//   if ((m_cols & (kRISCVVectorAlignment - 1)) != 0) {
//     unaligned = true;
//     aligned_row =
//         (int8_t*)aligned_alloc(kRISCVVectorAlignment, m_cols,  // NOLINT
//                                &aligned_row_free);
//   }
//   void* aligned_vec_free = nullptr;
//   int8_t* aligned_vec =
...
//   // vectorized form.
//   const int postamble_half_start =
//       RoundDownVectors<kInt8ValuesPerRISCVVector>(m_cols);
//   const int postamble_start =
//       RoundDownVectors<(kInt8ValuesPerRISCVVector / 2)>(m_cols);
...
//     RISCVCpuBackendGemm(vectors, bias, matrix, n_batch, m_cols, m_rows,
...
//     for (; j <= n_col - kInt8ValuesPerRISCVVector;
//          j += kInt8ValuesPerRISCVVector) {
...
//     }
//     int32_t sum = AccumulateRISCVLane(row_sum);
...
// void RISCVMatrixBatchVectorMultiplyAccumulateImpl(
//     const int8_t* __restrict__ matrix, const int m_rows, const int m_cols,
...
//     RISCVReductionSumVector(matrix, row_sums_ptr, m_rows, m_cols);
//   }
...
//       const int32_t* bias = static_cast<const int32_t*>(nullptr);
//       RISCVCpuBackendGemm(vectors, bias, matrix, n_batch, m_cols, m_rows, 0,
//                          scratch, context);
...
//   RISCVMatrixBatchVectorMultiplyAccumulateImpl(
//       matrix, m_rows, m_cols, vectors, scaling_factors, n_batch, result,
//       per_channel_scale, input_offset, row_sums);
// }
...
// void RISCVApplyLayerNorm(const int16_t* input, const int16_t* layer_norm_weights,
...
//       sum += static_cast<int64_t>(AccumulateRISCVLane(val_s32_0));
//       sum += static_cast<int64_t>(AccumulateRISCVLane(val_s32_1));

//       sum_sq += static_cast<int64_t>(
//           AccumulateRISCVLane(vmulq_s32(val_s32_0, val_s32_0)));
//       sum_sq += static_cast<int64_t>(
//           AccumulateRISCVLane(vmulq_s32(val_s32_1, val_s32_1)));
//     }
...
// void RISCVApplyTanhImpl(const int16_t* input, int32_t n_batch, int32_t n_input,
//                        int16_t* output) {
//   for (int batch = 0; batch < n_batch; ++batch) {
//     int i = 0;
// #ifdef GEMMLOWP_RISCV
...
//   case i:                                                  \
//     RISCVApplyTanhImpl<i>(input, n_batch, n_input, output); \
//     break;
//   switch (integer_bits) {
//     DISPATCH_TANH(0);
//     DISPATCH_TANH(1);
...
//   int i = 0;
//   for (; i <= v_size - kFloatValuesPerRISCVVector;
//        i += kFloatValuesPerRISCVVector) {
...
//   int i = 0;
//   for (; i <= v_size - kInt16ValuesPerRISCVVector * 2;
//        i += kInt16ValuesPerRISCVVector * 2) {
//     int16x8_t val_0 = vld1q_s16(vector + i);
//     int16x8_t val_1 = vld1q_s16(vector + i + kInt16ValuesPerRISCVVector);
...
//     vst1q_s16(vector + i + kInt16ValuesPerRISCVVector, val_1);
//   }
//   for (; TFLITE_UNLIKELY(i < v_size); i++) {
...
//   int i = 0;
//   for (; i < v_size - kInt8ValuesPerRISCVVector * 2;
//        i += kInt8ValuesPerRISCVVector * 2) {
//     int8x16_t val_0 = vld1q_s8(vector + i);
//     int8x16_t val_1 = vld1q_s8(vector + i + kInt8ValuesPerRISCVVector);
...
//     vst1q_s8(vector + i + kInt8ValuesPerRISCVVector, val_1);
//   }
//   for (; TFLITE_UNLIKELY(i < v_size); i++) {
//     vector[i] = std::max(std::min(clipping_value, vector[i]),
...
//   constexpr int kBlockSize = kFloatValuesPerRISCVVector;
...
//       }
//       result[batch * m_rows + row] += AccumulateRISCVLane(acc_32x4);
//     }
//   }
// }
...
//   constexpr int kBlockSize = kInt8ValuesPerRISCVVector;
//   TFLITE_DCHECK_EQ(m_cols % kBlockSize, 0);
...
//     float* __restrict__ result) {
//   constexpr int kRISCVVectorsPerBlock = 4;
//   constexpr int kBlockSize = kRISCVVectorsPerBlock * kFloatValuesPerRISCVVector;
//   TFLITE_DCHECK_EQ(  // NOLINT
//       m_cols % kBlockSize, 0);
...
//           for (int c = 0; c < kRISCVVectorsPerBlock; c++) {
//             // Load 4 float values from the vector and matrix row.
//             float32x4_t vector_f32x4 = vld1q_f32(vector_block_in_batch_ptr +
//                                                  c * kFloatValuesPerRISCVVector);
//             float32x4_t matrix_f32x4 =
//                 vld1q_f32(matrix_ptr + c * kFloatValuesPerRISCVVector);
//             // Multiply the vector and matrix row and add to accumulator.
//             acc_32x4 = vmlaq_f32(acc_32x4, matrix_f32x4, vector_f32x4);
//           }
//           matrix_ptr += kBlockSize;
...
//           // Otherwise, performance may suffer significantly.
//           TFLITE_DCHECK_EQ(  // NOLINT
//               (uintptr_t)(&row_ptr) & (kRISCVVectorAlignment - 1), 0);
...
//   // where this should happen.
//   const int postamble_start =
//       RoundDownVectors<kFloatValuesPerRISCVVector>(v_size);

//   float32x4_t one_f32x4 = vmovq_n_f32(1.0);
//   int v = 0;
//   for (; v < postamble_start; v += kFloatValuesPerRISCVVector) {
...
//   int postamble_start = RoundDownVectors<kInt16ValuesPerRISCVVector>(v_size);
...
//   int i = 0;
//   for (; i < postamble_start; i += kInt16ValuesPerRISCVVector) {
...
// #ifndef __SSE__
// // With Intel RISCV-2-SSE translator library, this is a redefinition..
...
//   // where this should happen.
//   const int postamble_start =
//       RoundDownVectors<kInt8ValuesPerRISCVVector>(v_size);

//   int v = 0;
//   for (; v < postamble_start; v += kInt8ValuesPerRISCVVector) {
...
//   // Here the assumption is that each buffer is 4-byte aligned.
//   TFLITE_CHECK_EQ((intptr_t)(&vector[0]) & (kRISCVVectorAlignment - 1), 0);
//   // If v_size is not divisible by kInt8ValuesPerRISCVVector, we cannot use the
...
// inline void RISCVMinMax(const float* values, const int size, float* min,
//                        float* max) {
//   const int postamble_start = RoundDownVectors<kFloatValuesPerRISCVVector>(size);
//   float rmin = 0.0f, rmax = 0.0f;
//   int i = 0;
//   if (postamble_start) {
//     float32x4_t min_f32x4 = vld1q_f32(values);
//     float32x4_t max_f32x4 = min_f32x4;
//     for (i = kFloatValuesPerRISCVVector; i < postamble_start;
//          i += kFloatValuesPerRISCVVector) {
...
//   *min = *minmax.first;
//   *max = *minmax.second;
//   RISCVSymmetricQuantizeFloats(values, size, quantized_values, *min, *max,
//                               scaling_factor);
// }
...
//   const int postamble_start =
//       RoundDownVectors<(2 * kFloatValuesPerRISCVVector)>(size);
...
//   int i = 0;
//   for (; i < postamble_start; i += 2 * kFloatValuesPerRISCVVector) {
...
//         vld1q_f32(&values[i + kFloatValuesPerRISCVVector]);
...
//   float rmin, rmax;
//   RISCVMinMax(values, size, &rmin, &rmax);
...
//   }
//   float result = AccumulateRISCVLane(acc_32x4);
//   // Postamble loop.
//   for (; TFLITE_UNLIKELY(v < v_size); v++) {
...
//     const int postamble_start =
//         RoundDownVectors<kFloatValuesPerRISCVVector>(reduction_size);
//     float32x4_t sum_f32x4 = vmovq_n_f32(0.0);
//     int r = 0;
//     for (; r < postamble_start; r += kFloatValuesPerRISCVVector) {
//       float32x4_t v1_f32x4 = vld1q_f32(input_vector + r);
//       sum_f32x4 = vaddq_f32(sum_f32x4, v1_f32x4);
//     }
//     float sum = AccumulateRISCVLane(sum_f32x4);
//     // Postamble loop.
...
//   const int postamble_half_start =
//       RoundDownVectors<kInt8ValuesPerRISCVVector>(reduction_size);
//   const int postamble_start =
//       RoundDownVectors<(kInt8ValuesPerRISCVVector / 2)>(reduction_size);
//   for (int o = 0; o < output_size; ++o) {
//     int32x4_t sum_32x4 = vmovq_n_s32(0);
//     int r = 0;
//     for (; r < postamble_half_start; r += kInt8ValuesPerRISCVVector) {
...
//       r += (kInt8ValuesPerRISCVVector >> 1);
//     }
//     int32_t sum = AccumulateRISCVLane(sum_32x4);
//     for (; TFLITE_UNLIKELY(r < reduction_size); ++r) {
//       sum += input_vector[r];
//     }
//     output_vector[o] = sum;
...
//                                  int n_batch) {
//   constexpr int kBlockSize = kFloatValuesPerRISCVVector * 4;
...
//           vld1q_f32(input_vector + i + 0 * kFloatValuesPerRISCVVector);
//       const float32x4_t input_f32x4_1 =
//           vld1q_f32(input_vector + i + 1 * kFloatValuesPerRISCVVector);
//       const float32x4_t input_f32x4_2 =
//           vld1q_f32(input_vector + i + 2 * kFloatValuesPerRISCVVector);
//       const float32x4_t input_f32x4_3 =
//           vld1q_f32(input_vector + i + 3 * kFloatValuesPerRISCVVector);
...
//     float sum = AccumulateRISCVLane(sum_f32x4_0);
//     for (; TFLITE_UNLIKELY(i < v_size); ++i) {
//       sum += input_vector[i];
//     }
//     // Calculate mean
...
//     float sum_diff_sq = AccumulateRISCVLane(sum_diff_sq_f32x4_0);
//     for (; TFLITE_UNLIKELY(i < v_size); ++i) {
...
//       vst1q_f32(output_vector + i + 0 * kFloatValuesPerRISCVVector,
//                 output_f32x4_0);
//       vst1q_f32(output_vector + i + 1 * kFloatValuesPerRISCVVector,
//                 output_f32x4_1);
//       vst1q_f32(output_vector + i + 2 * kFloatValuesPerRISCVVector,
//                 output_f32x4_2);
//       vst1q_f32(output_vector + i + 3 * kFloatValuesPerRISCVVector,
//                 output_f32x4_3);
//     }
//     for (; TFLITE_UNLIKELY(i < v_size); ++i) {
