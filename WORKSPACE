...
http_archive(
    ...
    patch_cmds = [
        "echo 'bazel_dep(name = \"rules_python\", version = \"1.8.1\")' >> MODULE.bazel",
        "sed -i '1i load(\"@rules_python//python:defs.bzl\", \"py_binary\")' refresh_compile_commands.bzl",
        "sed -i 's/native.py_binary/py_binary/g' refresh_compile_commands.bzl",
    ],
)
